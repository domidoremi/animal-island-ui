...
> **RN fork:** Button 在暗色模式使用 Provider 颜色，保留立体按压外观，忙碌时禁用操作，并遵守减少动态设置。 [RN contract](../../../RN-PORT.zh-CN.md). 其余表格描述上游 Web/亮色皮肤。

RN 加载文案与旋转图标描边共用前景色，确保与实心加载背景至少达到 4.5:1 对比度。亮色薄荷绿背景不变，但加载前景由上游白色改为深绿。暗色/自定义强调色按需选用深绿、白色或近黑墨绿；透明或无法解析的强调色回退到主题实心表面。加载外观优先于 link/danger/ghost 颜色及禁用透明度，但不会恢复交互。调用方通过 `style`/`textStyle` 覆盖颜色后需自行保证可读性。
