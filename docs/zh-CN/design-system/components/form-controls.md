...
> **RN fork:** Input 与 Select 使用暗色表面、文字和边框；Switch 使用暗色轨道与滑块。禁用选项不可选；紧凑控件以 hitSlop 扩大触控范围，不改变外观尺寸。 [RN contract](../../../RN-PORT.zh-CN.md). 其余表格描述上游 Web/亮色皮肤。
...
**RN fork / Web 宿主：** `Radio` 使用无样式的 `RadioGroup` 容器，每组只有一个 Tab
入口。方向键移动并激活选项，空格键在松开时选中，Home/End 定位首尾可用项，横向方向键遵循 RTL。
禁用项以及通过 `hidden`、`aria-hidden` 或 `inert` 隐藏的选项会被跳过。
自定义单选卡片可以复用同一个容器而不改变视觉；每个子项提供 `role="radio"`、
选中状态及原有的按压处理函数。嵌套单选组及可编辑后代保留自己的键盘事件。
选中值始终由宿主管理，延迟应用的主题过渡也不例外。原生宿主保留各选项独立的
无障碍节点；DOM 导航仅安装在 Web 上。不添加新动画或样式。
